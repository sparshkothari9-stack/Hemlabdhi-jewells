...
router.get('/stock', requireAuth, (req, res) => {
  const stock = all('SELECT product_id, stock, low_stock_threshold FROM product_stock');
  const map = {};
  for (const s of stock) map[s.product_id] = s;
  res.json({ stock: map });
});

router.put('/stock', requireAuth, requireAdmin, (req, res) => {
  ...
  for (const [productId, data] of Object.entries(stock)) {
    ...
    const existing = get('SELECT product_id FROM product_stock WHERE product_id = ?', [pid]);
    if (existing) {
      run('UPDATE product_stock SET stock = ?, low_stock_threshold = ? WHERE product_id = ?', [qty, threshold, pid]);
    } else if (qty !== null) {
      run('INSERT INTO product_stock (product_id, stock, low_stock_threshold) VALUES (?, ?, ?)', [pid, qty, threshold]);
    }
  }
...
// ===== ORDER NOTES =====
router.get('/orders/:id/notes', requireAuth, requireAdmin, (req, res) => {
  const orderId = parsePositiveInt(req.params.id);
  if (!orderId) return res.status(400).json({ error: 'Invalid order id' });
  const notes = all('SELECT * FROM order_notes WHERE order_id = ? ORDER BY created_at DESC', [orderId]);
  res.json({ notes });
});

router.post('/orders/:id/notes', requireAuth, requireAdmin, (req, res) => {
  const orderId = parsePositiveInt(req.params.id);
  if (!orderId) return res.status(400).json({ error: 'Invalid order id' });
  const note = asString(req.body.note, 1000);
  if (!note) return res.status(400).json({ error: 'Note required' });
  run('INSERT INTO order_notes (order_id, note) VALUES (?, ?)', [orderId, note]);
  res.json({ success: true });
});

// ===== TRACKING =====
router.put('/orders/:id/tracking', requireAuth, requireAdmin, (req, res) => {
  const orderId = parsePositiveInt(req.params.id);
  if (!orderId) return res.status(400).json({ error: 'Invalid order id' });
  const tracking = asString(req.body.tracking_number, 200);
  run('UPDATE orders SET tracking_number = ? WHERE id = ?', [tracking, orderId]);
  res.json({ success: true });
});
...
router.get('/dashboard', requireAuth, requireAdmin, (req, res) => {
  ...
  const totalRevenue = get('SELECT COALESCE(SUM(total),0) as c FROM orders WHERE status != "cancelled"')?.c || 0;
  const pendingOrders = get("SELECT COUNT(*) as c FROM orders WHERE status = 'pending'")?.c || 0;
  const totalClients = get('SELECT COUNT(*) as c FROM clients WHERE is_admin = 0')?.c || 0;
  const totalProducts = get('SELECT COUNT(*) as c FROM products')?.c || 0;
  const lowStock = all(`SELECT p.id, p.name, ps.stock FROM products p JOIN product_stock ps ON ps.product_id = p.id WHERE ps.stock <= ps.low_stock_threshold ORDER BY ps.stock LIMIT 5`);
  const recentOrders = all('SELECT id, client_name, total, status, created_at FROM orders ORDER BY created_at DESC LIMIT 5');
  const revenueByMonth = all(`SELECT strftime('%Y-%m', created_at) as month, COALESCE(SUM(total),0) as revenue FROM orders WHERE status != 'cancelled' GROUP BY month ORDER BY month DESC LIMIT 6`);
  const topProducts = all(`SELECT oi.product_name, SUM(oi.qty) as qty, SUM(oi.qty * oi.price) as revenue FROM order_items oi JOIN orders o ON o.id = oi.order_id WHERE o.status != 'cancelled' GROUP BY oi.product_name ORDER BY qty DESC LIMIT 5`);
...
router.post('/enquiries', (req, res) => {
  ...
  run('INSERT INTO enquiries (client_id, client_name, client_email, client_phone, products, message) VALUES (?, ?, ?, ?, ?, ?)',
    [clientId, name, email, phone, products, message]);
  res.json({ success: true });
});

router.get('/enquiries', requireAuth, requireAdmin, (req, res) => {
  const enquiries = all('SELECT * FROM enquiries ORDER BY created_at DESC');
  res.json({ enquiries });
});

router.put('/enquiries/:id', requireAuth, requireAdmin, (req, res) => {
...
router.get('/coupons', requireAuth, (req, res) => {
  if (req.client?.is_admin) {
    return res.json({ coupons: all('SELECT * FROM coupons ORDER BY created_at DESC') });
  }
  const coupons = all("SELECT code, discount_percent, min_amount FROM coupons WHERE is_active = 1 AND (expires_at IS NULL OR expires_at > datetime('now'))");
  res.json({ coupons });
});
...
router.post('/coupons', requireAuth, requireAdmin, (req, res) => {
  ...
  if (get('SELECT id FROM coupons WHERE code = ?', [code])) {
    return res.status(409).json({ error: 'Coupon code already exists' });
  }
  const created = run('INSERT INTO coupons (code, discount_percent, min_amount, max_uses, expires_at) VALUES (?, ?, ?, ?, ?)',
    [code, discount_percent, min_amount, max_uses, expires_at]);
  res.json({ coupon: get('SELECT * FROM coupons WHERE id = ?', [created.lastInsertRowid]) });
});

router.put('/coupons/:id', requireAuth, requireAdmin, (req, res) => {
  const id = parsePositiveInt(req.params.id);
  if (!id) return res.status(400).json({ error: 'Invalid id' });
  const existing = get('SELECT id FROM coupons WHERE id = ?', [id]);
  ...
  if (fields.length > 0) {
    vals.push(id);
    run(`UPDATE coupons SET ${fields.join(', ')} WHERE id = ?`, vals);
  }
  res.json({ coupon: get('SELECT * FROM coupons WHERE id = ?', [id]) });
});

router.post('/coupons/validate', requireAuth, (req, res) => {
  const code = asString(req.body.code, 32).toUpperCase().replace(/\s+/g, '');
  const amount = Math.max(0, parseFloat(req.body.amount) || 0);
  if (!code) return res.status(400).json({ error: 'Coupon code required' });
  const coupon = get(`SELECT * FROM coupons WHERE code = ? AND is_active = 1 AND (expires_at IS NULL OR expires_at > datetime('now'))`, [code]);
...
router.post('/coupons/:id/apply', requireAuth, requireAdmin, (req, res) => {
  ...
  run('UPDATE orders SET coupon_code = ? WHERE id = ?', [couponCode, orderId]);
  run("UPDATE coupons SET used_count = used_count + 1 WHERE code = ?", [couponCode]);
  res.json({ success: true });
});

// ===== EXPORT ORDERS CSV =====
router.get('/orders/export', requireAuth, requireAdmin, (req, res) => {
...
router.get('/orders/:id/invoice', requireAuth, (req, res) => {
  ...
  const notes = all('SELECT * FROM order_notes WHERE order_id = ?', [orderId]);
